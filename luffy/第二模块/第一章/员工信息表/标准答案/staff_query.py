...
def syntax_where(clause):
    """
    # where 右边赋值给where_clause == clause,例如dept = 'IT'
    解析where条件，并查询数据
    ...
    for op_key,op_func in operators.items():
        if op_key in clause:#op_key 符号
            q_name,q_condition = clause.split(op_key)# 例如dept  'IT'
            # print("query:",q_name,q_condition)
            if q_name.strip() in STAFF_DATA:
                matched_data = op_func(q_name.strip(),q_condition.strip()) #调用对应的方法 例如： op_gt(dept,IT)
                return matched_data
            else:
...
def syntax_parser(cmd):
    ...
    if cmd.split()[0] in ['find','add','update','delete'] and "staff_table" in cmd :
        # 所有带where 有查、删、修改
        if 'where' in cmd:
            query_cmd,where_clause = cmd.split("where")# 以where为切割 左边赋值给 query_cmd，，右边赋值给where_clause

            matched_data = syntax_where(where_clause.strip()) # 调用 def syntax_where(clause):
