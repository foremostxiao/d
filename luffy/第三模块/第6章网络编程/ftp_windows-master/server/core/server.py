        ...
        while True:
            ...
            if user_data:
                if user_data[0][1] == hashlib.md5(user_dic.get('password').encode('utf-8')).hexdigest():  # 密码也相同
                    ...
                    conn.send(pickle.dumps(user_info_dic))  # 用户的详细信息发送到客户端
    ...
    def ls(self,conn):
        ...
        if len(self.cmds) > 1:
            ...
            if os.path.isdir(file_path):
                ...
                subpro_obj = subprocess.Popen('dir', shell=True,  # 这里如果没有输入正确的目录名 就默认显示当前目录
                                              stdout=subprocess.PIPE,
                                              stderr=subprocess.PIPE)
        else:
    ...
    def cd(self,conn):
        ...
        if len(self.cmds) > 1:
            ...
            if os.path.isdir(dir_path):
                ...
                # 写成这样为了防止 ../  否则会有ftp_2\server\home\lily\test5\../../ 这样的情况存在
